...
function replyMessage(id){
	...
	str = reply_obj.text();
	str1 = reply_btn_obj.text();
	if(str1 == "提交"){
		value = $("#reply_input_"+id).attr("value");
		$.post("/admin/message/aj_reply.php", {id: id, reply: value },
				  function(data){
...
function deleteMessage(id){
	re = delConfirm();
	if(!re)return;
	$.post("/admin/message/aj_delete.php", {id:id},
			  function(data){
					if(data == 'M0001'){							
...
function deleteDownload(id){
	re = delConfirm();
	if(!re)return;
	$.post("/admin/download/aj_delete.php", {id:id},
			  function(data){
					if(data == 'M0001'){							
